    ...
    def checkAllPhasesEnd():
        ...
        if check == 8:  # 全部時相都結束了，將相關list設定回原始預設值
            ...
            for num in range(0,4):  # cycle結果累計 1. 綠燈長度
                ...
                RSUs['I1'].CycleAccumulated = RSUs['I1'].CycleAccumulated + GijkResult[num]
            CycleAccumulated += R*4  # cycle結果累計 1. 黃燈 + 全紅
            RSUs['I1'].CycleAccumulated += R*4
            print("CycleAccumulated = ", CycleAccumulated)
            print("RSUs['I1'].CycleAccumulated = ", RSUs['I1'].CycleAccumulated)
...
def EXE_GUROBI():
    ...
    monitorGijkLength()  # 1. 更新綠燈執行狀態
    if checkNowStateYorAR():  # 2. 檢查所有時相是否在黃燈or全紅時間
    ...
    try:
        ...
        for j in phaseStrDict:
            ...
            for x in range(IQj[j] + 1, IQj[j] + 1 + len(vehNoneQueue[phaseStrDict[j]])):
                ...
                m.addConstr(TPjx[j][x] >= Tijk[j] + GijkLengthCal[j] * (1 - phaseEndBinaryVar[j]) + tijk[j] - Ykjx[j, x] * M + 0.00001, 'NQC_1')
                m.addConstr(TPjx[j][x] <= Tijk[j] + GijkLengthCal[j] * (1 - phaseEndBinaryVar[j]) + tijk[j] + (1 - Ykjx[j, x]) * M, 'NQC_2')
                ...
                m.addConstr(TAjx[j, x] == (Tijk[j] + GijkLengthCal[j] * (1 - phaseEndBinaryVar[j]) + tijk[j]))
        ...
        m.addConstr(Tijk[1] == Tijk[0] + tijk[0] + R + (GijkLengthCal[0] * (1 - phaseEndBinaryVar[0])), 'ct5')  # Rhoijk[0]
        m.addConstr(Tijk[2] == Tijk[1] + tijk[1] + R + (GijkLengthCal[1] * (1 - phaseEndBinaryVar[1])), 'ct6')  # Rhoijk[1]
        m.addConstr(Tijk[3] == Tijk[2] + tijk[2] + R + (GijkLengthCal[2] * (1 - phaseEndBinaryVar[2])), 'ct7')   # Rhoijk[2]
        m.addConstr(Tijk[5] == Tijk[4] + tijk[4] + R + (GijkLengthCal[4] * (1 - phaseEndBinaryVar[4])), 'ct8')  # Rhoijk[4]
        m.addConstr(Tijk[6] == Tijk[5] + tijk[5] + R + (GijkLengthCal[5] * (1 - phaseEndBinaryVar[5])), 'ct9')  #Rhoijk[5]
        m.addConstr(Tijk[7] == Tijk[6] + tijk[6] + R + (GijkLengthCal[6] * (1 - phaseEndBinaryVar[6])), 'ct10') #Rhoijk[6]
        m.addConstr(C >= Cmin ,'cCmin')
        m.addConstr(C <= Cmax - 0.00001, 'cCmax')
        m.addConstr(Tijk[3] + tijk[3] + R + (GijkLengthCal[3] * (1 - phaseEndBinaryVar[3])) - Tijk[0] == C, 'cC1') # Rhoijk[3]
        m.addConstr(Tijk[7] + tijk[7] + R + (GijkLengthCal[7] * (1 - phaseEndBinaryVar[3])) - Tijk[4] == C, 'cC2') # Rhoijk[7]
...
def getVehicleParameters(vehID):
    ...
    TPxj = dist/Vlim + traci.simulation.getTime() - CycleAccumulated + 2
...
def initialization():
    # 初始化: 新增RSU
    I1 = RSUObject.RSU(ID='I1', location=[300, 0])
    RSUs.update({'I1': I1})
    Plan = ([9, 5, 18, 5, 9, 5, 18, 5], [0, 14, 24, 47, 0, 14, 24, 47],
...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if step in range(0, 7200, 1):
            ...
            #TLS_program = PhaseObject.NEWmakeLogicObject(currentPhase, tijkResult, phasePendingCounter)
